...
def perform_lasso_cv_with_mse(X, y, feature_names, mark_topk=5, cv=5, max_iter=20000,visualize=True):
    """
...
def prepare_loess_model(x, y, bins=300, frac=0.013, visualize=True):
...
def get_distance_scaler(genei, genes, results, attention_scores, distances, receiver_type, count_threshold=100,visualize=True):
    ...
    model = prepare_loess_model(distances_flatten, scores, visualize=visualize)
