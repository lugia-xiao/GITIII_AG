...
class Spatial_visualizer():
    def __init__(self,sample):
        ...
        type_exps = torch.Tensor(np.stack([type_exp_dict[cell_typei][np.array(self.target_gene_index)] for cell_typei in cell_type_target], axis=0))
    ...
    def read_prediction_adata(self,state=False):
        ...
        type_exps = np.stack([type_exp_dict[cell_typei][np.array(self.target_gene_index)] for cell_typei in cell_type_target], axis=0)
