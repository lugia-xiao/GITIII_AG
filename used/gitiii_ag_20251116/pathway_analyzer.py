...
class Pathway_analyzer():
    ...
    def find_significant_LR_with_known_receiver_value__scaled_LR_VS_cell_state(self,
        ...
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_pred = perform_lasso_cv_with_mse(X=signal_strengthi, y=y_pred_DL,
    ...
    def visualize_calculate_LR(self,ligand_genes,receptor_genes,receiver_cell_type,target_gene):
        ...
        for cell_index in range(proportioni.shape[0]):
            if self.cell_type_target[cell_index] == receiver_cell_type:
                ...
                receiver_cell = self.adata.X[indices[0]:indices[0]+1, :]
                sender_cells = self.adata.X[indices[1:], :]
        ...
        self.adata_receiver.obs['-'.join(ligand_genes)+'-->'+'-'.join(receptor_genes)+'-->'+target_gene] = LR_datas[flags]
    ...
    def visualize_LR(self,ligand_gene,receptor_gene,receiver_cell_type,target_gene):
        ...
        if ligand_gene+"->"+receptor_gene in self.signal_names:
            plot_with_correlations(
                ...
                y=np.array(self.adata_receiver[:, target_gene+"_st"].X.flatten()),
    ...
    def find_significant_LR__LR_VS_predicted_influence__only_receiver_type(self,receiver_type, targeted_gene,
        ...
        print("Doing LASSO analysis")
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_pred = perform_lasso_cv_with_mse(signal_strengthi, y, feature_names=self.signal_names,max_iter=max_iter,visualize=visualize)
...
class Pathway_analyzer_prepared():
    def __init__(self,adata,st_sample,species="human",sc_label=None,st_label=None,
                 filter_noise_proportion=0.02,discard_no_match_threshold=0.7,num_neighbors=50,
                 genes_to_remove_from_LR=None):
        assert species in ["human", "mouse"], "Species must be one of human or mouse"
        ...
        all_genes = torch.load(os.path.join(os.getcwd(), "data", "genes.pth"))
        self.all_st_genes = all_genes
        genes = torch.load(os.path.join(os.getcwd(), "data", "target_genes.pth"))
        target_gene_index = []
        for i in range(len(all_genes)):
            if all_genes[i] in genes:
                target_gene_index.append(i)
        ...
        print("Number of genes in the integrated adata:",len(self.all_genes))
        ...
        genes_to_search=self.all_genes
        if genes_to_remove_from_LR is not None:
            genes_to_search=[i for i in self.all_genes if i not in genes_to_remove_from_LR]
        self.interactions=search_interactions_LR(genes=genes_to_search,species=species)
        ...
        self.adata=adata
        self.adata.obs["have_match"]=np.array([True for i in range(self.adata.shape[0])])
        self._validate_obs_names()
        self._add_missing_obs_columns()

    def _validate_obs_names(self):
        # First check: same number of cells
        if len(self.st_adata.obs_names) != len(self.adata.obs_names):
            raise ValueError(
                f"Number of cells mismatch: st_adata has {len(self.st_adata.obs_names)}, "
                f"adata has {len(self.adata.obs_names)}"
            )

        # Second check: same names and order
        if not self.st_adata.obs_names.equals(self.adata.obs_names):
            raise ValueError("Cell names (obs_names) do not match between st_adata and adata.")

    def _add_missing_obs_columns(self):
        # find columns in st_adata.obs that are not in adata.obs
        missing_cols = [col for col in self.st_adata.obs.columns if col not in self.adata.obs.columns]

        # reindex each missing column to adata.obs_names (align by cell IDs)
        for col in missing_cols:
            self.adata.obs[col] = self.st_adata.obs[col].reindex(self.adata.obs_names)
    ...
    def find_significant_LR_with_known_receiver_value__scaled_LR_VS_cell_state(self,
        ...
        y_pred_DL=y_pred_DL[flag_ij]
        print("Doing LASSO analysis")
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_pred = perform_lasso_cv_with_mse(X=signal_strengthi, y=y_pred_DL,#y,
                                                                                     feature_names=self.signal_names,
                                                                                     max_iter=max_iter)
    ...
    def find_significant_LR__LR_VS_predicted_influence__receiver_sender(self, receiver_type, sender_type_list,
        ...
        print("Now calculating which pathway significantly influence the gene of", targeted_gene, "in ", receiver_type)
        print("LR_VS_predicted_influence__receiver_sender")
        self.current_LIME = "Sender cell types in [" + ", ".join(
            sender_type_list) + "] ->" + receiver_type + "--" + targeted_gene
        ...
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_pred = perform_lasso_cv_with_mse(
            signal_strengthi, y, feature_names=self.signal_names, max_iter=max_iter)
        ...
        self.LIME[targeted_gene + "_" + "pred"] = y_pred
        for i in range(len(significant_LR.tolist())):
            self.LIME[targeted_gene + "_" + str(significant_LR.tolist()[i])] = signal_strengthi[:,
                                                                               self.signal_names.index(
                                                                                   significant_LR.tolist()[i])]
        if visualize:
            plot_with_correlations(x=y_pred, y=y, label_x="Reconstructed influence", label_y="DL-estimated influence")
