...
if __name__=="__main__":
    ...
    special_modules = [
        my_model.embeddings.distance_scaler,
        my_model.embeddings.distance_embedding1,
        my_model.embeddings.distance_embedding2,
    ]

    # Flatten all parameters from the special modules
    special_params = list(
        itertools.chain.from_iterable(m.parameters() for m in special_modules)
    )

    # Use ids to avoid weirdness with tensor equality
    special_param_ids = {id(p) for p in special_params}

    # All other parameters = base group
    base_params = [p for p in my_model.parameters() if id(p) not in special_param_ids]

    optimizer = torch.optim.AdamW(
        [
            {"params": special_params, "lr": 1e-1},
    ...
    for i in range(1):
        ...
        start = time.time()
        for i in range(3):
            x=torch.load("../data/example_data.pth")
            '''{
