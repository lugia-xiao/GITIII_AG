...
from gitiii_ag.find_ligands import search_ligands
...
def get_index(df, num_neighbor, threshold):
    ...
    for i in df.index:
        ...
        flagi2 = (df.loc[i, "subclass"] != 'Unlabeled' and df.loc[i, "subclass"] !='unassigned')
...
def preprocess_dataset(df_all,genes,use_log_normalize,
                       library_size_normalize=False,target_sum=1e4,
                       species='human',use_nichenetv2=True,
    ...
    if library_size_normalize:
        print("Doing library size normalization")
        X = df_all.loc[:, genes].values
        if target_sum is None:
            target_sum = np.median(np.sum(X,axis=-1))
            print("Using target sum", target_sum)
        X = X/np.sum(X, axis=-1, keepdims=True)*target_sum
        df_all.loc[:, genes] = X

    # Do log(x+1) transform
    if use_log_normalize:
        print("Doing log2(x+1) tranform")
        X=df_all.loc[:,genes].values
        X=np.log(X+1)
        df_all.loc[:,genes]=X
        print("Finish doing log(x+1) tranform")

    # save cell types dictionary
    cell_types=np.unique(df_all.loc[:,"subclass"].values).tolist()
    print("Unique cell types:", cell_types)
    if len(cell_types)>40:
        raise ValueError("You have",len(cell_types),"cell types! That is too much! Try less than 40 cell types")
