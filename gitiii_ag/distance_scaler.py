...
class FFN(nn.Module):
    def __init__(self,dim,in_dim=None,out_dim=None,bias=False):
        super().__init__()
        if in_dim is None:
            in_dim=dim
        if out_dim is None:
...
class Monotonic_distance_embedding(nn.Module):
    def __init__(self, out_features, bias=True):
        ...
        self.input_dim = 3  # Four input features: -log(distance+1), -distance, -distance**2, 1/distance

        lip_nn = nn.Sequential(
            lmn.LipschitzLinear(self.input_dim, self.out_features * 4, kind="one-inf", bias=bias),
            lmn.GroupSort(self.out_features),
        ...
        # Monotonic constraints: all 4 input features should be monotonic decreasing
        # Each feature contributes to monotonic decreasing output
        self.monotonic_nn = lmn.MonotonicWrapper(lip_nn,
                                                 monotonic_constraints=[[1 for i in range(out_features)] for j in
    ...
    def forward(self, distance):
        ...
        # Stack multiple features: -log(distance+1), -distance, -distance**2, 1/distance
        # All features are monotonic decreasing with distance
        feature1 = -torch.log(distances + 1)  # shape: (B,)
        feature2 = -distances / 10  # shape: (B,)
        feature3 = -(distances ** 2) / 200  # shape: (B,)

        # Stack features: shape (B, 4)
        x = torch.stack([feature1, feature2, feature3], dim=1)

        x = self.monotonic_nn(x)  # shape: (B, out_features)
        return x.reshape(shape + [self.out_features])  # Output shape: (B, out_features)


class Monotonic_distance_embedding_not_used(nn.Module):
    def __init__(self, out_features, bias=True):
        super().__init__()
        self.out_features = out_features
