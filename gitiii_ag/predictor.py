...
from gitiii_ag.dataloader import GITIII_evaluate_dataset

def Calculate_influence_tensor(num_neighbors=50,batch_size=128,node_dim=256,
                               edge_dim=48,att_dim=8,num_heads=2,
                               use_cell_type_embedding=False):
    ...
    target_genes=torch.load("/".join(data_dir.split("/")[:-2]) + "/target_genes.pth")
    ...
    my_model = GITIII(genes=genes, target_genes=target_genes, ligands_info=ligands_info,
                      ...
                      node_dim_small=16, att_dim=att_dim,
                      use_cell_type_embedding=use_cell_type_embedding)
    my_model.eval()
    my_model = my_model.cuda()
    my_model.load_state_dict(torch.load(os.path.join(os.getcwd(),"GITIII_best.pth")))

    loss_func=Loss_function(gene_list=genes,target_genes=target_genes)
