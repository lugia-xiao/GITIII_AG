...
class Embedding(nn.Module):
    def __init__(self,genes,ligands_info,node_dim,edge_dim,use_cell_type_embedding=True):
        ...
        self.cell_encoder2 = nn.Embedding(40,node_dim)
    ...
    def forward(self,x):
        ...
        #print("Distance scaler",distance_scaler)

        # ligand score scaled by distance
        #print("start embdedding edge computation")
        #print(distance_scaler * data_ligand)
        edges=self.ligands_encoder((distance_scaler*data_ligand).unsqueeze(dim=1))#B,N,C->B,1,N,C
        #print("\n"*10,edges)
        #print("End embdedding edge computation")#print("weight",self.ligands_encoder.linear2.weight)

        #print("distance_scaler*data_ligand",distance_scaler*data_ligand)
        #print("edges",self.ligands_encoder((distance_scaler*data_ligand).unsqueeze(dim=1)))
