...
from gitiii_ag.pathway_analyzer import Pathway_analyzer
...
class Pathway_analyzer_multi_samples():
    ...
    def find_significant_LR_with_known_receiver_value__scaled_LR_VS_cell_state(self,
        ...
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_preds = perform_lasso_cv_with_mse(
            X=signal_strengths, y=ys,#_pred_DLs,#ys,
            weights=weights,
            feature_names=self.signal_names,
            max_iter=max_iter)

        torch.save([self.signal_names, indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_preds],
                   os.path.join(os.getcwd(), "results",
                                correct_string(str(receiver_type) + "__" + str(targeted_gene) + "_method1.pth")))
        ...
        if len(significant_LR.tolist())>0:
        ...
        for i in range(min(len(significant_LR),10)):
            print("Now visualizing:",significant_LR[i])
            plot_with_correlations(x=signal_strengths[:,self.signal_names.index(significant_LR[i])], y=ys, label_x=significant_LR[i], label_y="Measured expression of " + targeted_gene)
            print("-"*20)
            print("-" * 20)
            print("-" * 20)
    ...
    def find_significant_LR__LR_VS_predicted_influence__only_receiver_type(self,receiver_type, targeted_gene,
        ...
        signal_strengths, ys, weights=[], [], []
        for pathway_analyzer in self.list_of_pathway_analyzer:
            signal_strengthi, yi, weighti=pathway_analyzer.find_significant_LR__LR_VS_predicted_influence__only_receiver_type_multi_samples_first_half(
                receiver_type=receiver_type,targeted_gene=targeted_gene
            )
            signal_strengths.append(signal_strengthi)
            ys.append(yi)
        ...
        indices_to_zero_order, ranked_indices, coefficients, best_alpha, y_pred = perform_lasso_cv_with_mse(
            signal_strengths, ys, feature_names=self.signal_names, max_iter=max_iter, weights=weights)
